...
/* ROSE-TOOLS BEGIN: Win32/C89 build shims for non-GLib, non-EventManager path.
 *
 * We compile the non-GLib, non-vSocket path verbatim. The only structural
 * dependencies that path has beyond message.c are:
 *
 *   1. VMTOOLS_USE_GLIB / VMTOOLS_USE_VSOCKET  -- must be undefined.
 *   2. EventManager_Add / EventManager_Remove  -- stubbed: in->nextEvent is
 *      used only as a NULL/non-NULL sentinel. Actual scheduling is done by
 *      the caller (RpcIn_Poll in toolsRpc.c) via the Sleep-based main loop.
 *   3. DblLnkLst_Links                         -- opaque forward decl only.
 *   4. System_GetTimeMonotonic                 -- stubbed via GetTickCount.
 *   5. StrUtil_GetNextToken                    -- implemented below.
 *   6. GUEST_RPC_UNKNOWN_COMMAND               -- defined below.
 *   7. RpcIn_ErrorFunc / RpcIn_ClearErrorFunc  -- defined below.
 *   8. RpcIn_OldCallback (old-style)              -- defined below; distinct from
 *      the guestrpc.h new-style RpcIn_OldCallback.
 *
 * Everything else (Bool, uint64, ASSERT, FMTSZ, Debug, Message_*) is already
 * present in the tree.
 *
 * DO NOT remove or reorder these defines -- they must precede all includes.
 */
#undef  VMTOOLS_USE_GLIB
#undef  VMTOOLS_USE_VSOCKET

/* ROSE-TOOLS BEGIN: standard includes needed by the shim block itself.
 * The shim precedes the normal include block so size_t, NULL, malloc,
 * strchr etc. must be pulled in explicitly here.
 * ROSE-TOOLS ORIGINAL: provided transitively via eventManager.h etc. */
#include <stddef.h>
#include <string.h>
#include <stdlib.h>
/* ROSE-TOOLS END */

/* Stub for EventManager: nextEvent is a sentinel only, never dereferenced. */
typedef void *Event;
/* ROSE-TOOLS BEGIN: guard against redefinition - rpcin.h also declares these.
 * ROSE-TOOLS ORIGINAL: no guard (single TU assumed). */
#ifndef DBLLNKLST_LINKS_DEFINED
#define DBLLNKLST_LINKS_DEFINED
typedef struct { int _unused; } DblLnkLst_Links;
#endif
/* ROSE-TOOLS END */
#define EventManager_Add(queue, delay, fn, data)  ((Event *)(size_t)1)
#define EventManager_Remove(ev)                   ((void)0)
/* ROSE-TOOLS BEGIN: non-static so rpcin.h extern declaration resolves.
 * ROSE-TOOLS ORIGINAL: static DblLnkLst_Links gRoseTimerQueueStorage; */
DblLnkLst_Links gRoseTimerQueueStorage;   /* passed to RpcIn_Construct,
                                            * value is ignored        */
/* ROSE-TOOLS END */

/* ROSE-TOOLS BEGIN: System_GetTimeMonotonic stub moved after vm_basic_types.h
 * so uint64 is defined. See stub below the includes block.
 * ROSE-TOOLS ORIGINAL: static uint64 System_GetTimeMonotonic(void)
 *   { return (uint64)GetTickCount(); }
 */
/* ROSE-TOOLS END */

/* GUEST_RPC_UNKNOWN_COMMAND -- reply sent when no callback matches. */
#define GUEST_RPC_UNKNOWN_COMMAND "Unknown command"

/* Old-style RpcIn callback signature (non-GLib path).
 * Distinct from the new-style RpcIn_OldCallback in guestrpc.h. */
...
/* Error / clear-error function pointer types. */
typedef void (RpcIn_ErrorFunc)(void *data, char const *errmsg);
typedef void (RpcIn_ClearErrorFunc)(void *data);

/* StrUtil_GetNextToken -- extracts the next whitespace-delimited token.
 * Mirrors open-vm-tools strutil.c behaviour used in RpcInExecRpc.
 * Caller must free the returned string. Returns NULL at end of input. */
static char *
StrUtil_GetNextToken(unsigned int *index,
                     const char   *str,
                     const char   *delimiters)
{
   ...
   while (str[*index] != '\0' && strchr(delimiters, str[*index]) != NULL) {
      (*index)++;
   }
   if (str[*index] == '\0') {
      return NULL;
...
#   include "debug.h"
#   include "str.h"
/* ROSE-TOOLS BEGIN: strutil.h not in tree; StrUtil_GetNextToken implemented
 * inline in the shim block above. ROSE-TOOLS ORIGINAL:
 *   #include "strutil.h"
 */
/* ROSE-TOOLS END */
#endif
...
/* ROSE-TOOLS BEGIN: windows.h and System_GetTimeMonotonic placed here so
 * uint64 (from vm_basic_types.h) is defined before use.
 * ROSE-TOOLS ORIGINAL: system.h provided System_GetTimeMonotonic. */
#ifndef _WINDOWS_
#include <windows.h>
#endif
static uint64
...
{
   return (uint64)(DWORD)GetTickCount();
}
/* ROSE-TOOLS END */
...
/* ROSE-TOOLS BEGIN: tclodefs.h not in tree; defines TCLO protocol number
 * 0x4f4c4354 used in RpcInOpenChannel. Defined inline here.
 * ROSE-TOOLS ORIGINAL:
 *   #include "vmware/guestrpc/tclodefs.h"
 */
/* TCLO protocol cookie, from tclodefs.h */
#ifndef TCLO_PROTOCOL_NUM
#define TCLO_PROTOCOL_NUM  0x4f4c4354UL
#endif
/* ROSE-TOOLS END */
#include "vmware.h"
#include "message.h"
#include "rpcin.h"
/* ROSE-TOOLS BEGIN: util.h and system.h not in tree.
 * util.h provided Util_SafeStrdup (not used in non-GLib path).
 * system.h provided System_GetTimeMonotonic (stubbed in shim block above).
 * ROSE-TOOLS ORIGINAL:
 *   #include "util.h"
 *   #include "system.h"
 */
...
#if !defined(VMTOOLS_USE_GLIB)
/* ROSE-TOOLS BEGIN: eventManager.h replaced by stub macros in shim block above.
 * gTimerEventQueue replaced by gRoseTimerQueueStorage (ignored by stub).
 * ROSE-TOOLS ORIGINAL:
 *   #include "eventManager.h"
 *   static DblLnkLst_Links *gTimerEventQueue;
 */
static DblLnkLst_Links *gTimerEventQueue = &gRoseTimerQueueStorage;
/* ROSE-TOOLS END */
...
/* ROSE-TOOLS BEGIN: return type changed Bool -> unsigned int to match
 * RpcIn_OldCallback typedef. On this target Bool=char != unsigned int.
 * ROSE-TOOLS ORIGINAL: static Bool */
static unsigned int
...
/* ROSE-TOOLS END */
{
   /* ROSE-TOOLS BEGIN: suppress unused-parameter warnings.
    * ROSE-TOOLS ORIGINAL: (no suppressors) */
          ...
          int flags)   // IN
{
   ...
   ASSERT(in);
   ASSERT(in->mustSend);
   /* ROSE-TOOLS BEGIN: suppress unused 'flags' (vSocket path only).
    * ROSE-TOOLS ORIGINAL: (no suppressor) */
   (void)flags;
   /* ROSE-TOOLS END */
                 ...
                 Bool useBackdoorOnly)      // IN
{
   ...
   /* ROSE-TOOLS BEGIN: suppress unused 'useBackdoorOnly' (only used in
    * vSocket path above, which is compiled out). ROSE-TOOLS ORIGINAL: none */
   (void)useBackdoorOnly;
   /* ROSE-TOOLS END */
   ASSERT(in->channel == NULL);
   /* ROSE-TOOLS BEGIN: use named constant for TCLO protocol number.
    * ROSE-TOOLS ORIGINAL:   in->channel = Message_Open(0x4f4c4354); */
   in->channel = Message_Open(TCLO_PROTOCOL_NUM);
   /* ROSE-TOOLS END */
   if (in->channel == NULL) {
...
RpcIn_start(RpcIn *in,                                // IN
            unsigned int delay,                       // IN
            RpcIn_OldCallback resetCallback,             // IN
...
/* ROSE-TOOLS BEGIN: RpcIn_Poll -- not in upstream rpcin.c.
 * The non-GLib path uses EventManager to schedule RpcInLoop callbacks.
 * We have no EventManager; instead the main loop calls RpcIn_Poll every tick.
 * This function drives one iteration of the TCLO receive loop.
 * Returns TRUE if the channel is healthy, FALSE if it has stopped. */
Bool
RpcIn_Poll(RpcIn *in)
{
   if (in == NULL || in->nextEvent == NULL) {
      return TRUE;   /* not started -- not an error */
   }
   return RpcInLoop(in);
}
/* ROSE-TOOLS END */
