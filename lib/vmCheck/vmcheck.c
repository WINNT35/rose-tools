...
#  include "windows.h"
/* ROSE-TOOLS BEGIN: added setjmp.h for VEH-based safe call implementation */
#  include <setjmp.h>
/* ROSE-TOOLS END */
#endif
...
/* ROSE-TOOLS BEGIN: hostinfo.h and x86cpuid.h excluded on _WIN32,
 * str.h kept for Str_Strcmp used in Linux path only */
#  if !defined(_WIN32)
#     include "hostinfo.h"
#     include "x86cpuid.h"
#  endif
...
/* ROSE-TOOLS BEGIN: VEH handler and jump buffer for Win32 safe backdoor probe.
 * GCC/MinGW does not support MSVC __try/__except keywords.
 * AddVectoredExceptionHandler + setjmp/longjmp is the equivalent on Win32. */
#if defined(_WIN32)
static jmp_buf  g_roseJmpBuf;
static volatile int g_roseJmpValid = 0;

static LONG WINAPI
RoseVehHandler(EXCEPTION_POINTERS *ep)
{
   DWORD code = ep->ExceptionRecord->ExceptionCode;
   if ((code == EXCEPTION_PRIV_INSTRUCTION  ||
        code == EXCEPTION_ILLEGAL_INSTRUCTION ||
        code == EXCEPTION_ACCESS_VIOLATION) && g_roseJmpValid) {
      g_roseJmpValid = 0;
      longjmp(g_roseJmpBuf, 1);
   }
...
#if defined(_WIN32)
/* ROSE-TOOLS BEGIN: replaced MSVC __try/__except with VEH + setjmp/longjmp.
 * __try/__except is not supported by GCC/MinGW. */
   {
      PVOID handler = AddVectoredExceptionHandler(1, RoseVehHandler);
      if (setjmp(g_roseJmpBuf) == 0) {
         g_roseJmpValid = 1;
         result = checkFn();
         g_roseJmpValid = 0;
      }
...
/* ROSE-TOOLS BEGIN: Hostinfo_* and CPUID hypervisor checks excluded on Win32.
 * On Windows we go straight to the backdoor probe via VmCheckSafe.
 * The Linux path checks for Xen/VirtualPC first which requires hostinfo.h
 * and x86cpuid.h - neither of which are needed or available in our Win32 build. */
#if defined VM_X86_ANY && !defined(_WIN32)
   {
      char *hypervisorSig;
      uint32 i;
...
#endif /* VM_X86_ANY && !_WIN32 */
/* ROSE-TOOLS END */

/* ROSE-TOOLS BEGIN: Hostinfo_TouchBackDoor excluded on _WIN32 */
#if !defined(_WIN32)
