...
typedef struct RoseAppCtx {
   ...
   RpcChannel        *rpc;
   /* ROSE-TOOLS BEGIN: inbound TCLO channel added alongside outbound rpc.
    * rpcIn is driven by RoseToolsRpc_Poll() in the main loop.
    * NULL when not running in a VMware VM. */
   struct RpcIn      *rpcIn;
   /* ROSE-TOOLS END */
} RoseAppCtx;
/* -----------------------------------------------------------------------
 ...
 * --------------------------------------------------------------------- */
typedef RosePluginData *(*RoseOnLoadFn)(RoseAppCtx *ctx);

/* -----------------------------------------------------------------------
 * Service name constants
 * --------------------------------------------------------------------- */
/* ROSE-TOOLS BEGIN: VMware Workstation 15.5 and earlier use the legacy
 * "toolbox" channel name. "vmtoolsd" was introduced in later versions.
 * Using the wrong name causes ToolsGetAppGenericName to fail and
 * VMware to skip Capabilities_Register entirely.
 * ROSE-TOOLS ORIGINAL: "vmrosd" */
#define ROSE_GUEST_SERVICE  "toolbox"
/* ROSE-TOOLS END */
